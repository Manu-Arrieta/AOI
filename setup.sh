...
mkdir -p "$PROJECT_PATH/apps/agentic-ops-dashboard/app/components"
mkdir -p "$PROJECT_PATH/apps/agentic-ops-dashboard/app/pages"
mkdir -p "$PROJECT_PATH/apps/agentic-ops-dashboard/server/api"
mkdir -p "$PROJECT_PATH/apps/agentic-ops-dashboard/server/routes"
mkdir -p "$PROJECT_PATH/apps/agentic-ops-dashboard/server/utils"
mkdir -p "$PROJECT_PATH/apps/agentic-ops-dashboard/shared"
mkdir -p "$PROJECT_PATH/apps/agentic-ops-dashboard/test"
ok "Directories: .tasks/ .sandboxes/ .atl/ .resources/ apps/agentic-ops-dashboard/"
...
echo "    2. Run /speckit.constitution in Copilot Chat"
echo "    3. Start your first cycle: /sdd-new"
if [ -f "$PROJECT_PATH/pnpm-workspace.yaml" ]; then
  echo "    4. Start the dashboard runtime: pnpm dev:dashboard"
fi
echo ""
