...
export const GENERATED_BANNER = '<!-- generated:do-not-edit — source: integration-manifest.json -->'
...
export function renderManifestMarkdown(manifest) {
  ...
  const compartments = Array.isArray(manifest.compartments) ? manifest.compartments : []
  ...
  lines.push(`_Generated ${generatedAt} — DO NOT EDIT (regenerated from JSON)._`)
  lines.push('')
  ...
  for (const compartment of compartments) {
    const own = elements.filter((element) => element.compartment === compartment.id)
    lines.push('')
...
async function main() {
  ...
  if (!filePath) {
    process.stderr.write('usage: node generate-manifest-md.mjs <path-to-integration-manifest.json> [--stdout]\n')
    process.exitCode = 1
    return
  }
