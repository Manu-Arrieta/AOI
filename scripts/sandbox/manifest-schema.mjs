...
export const allowedKinds = new Set(['frontend', 'backend', 'api-contract', 'data', 'infra', 'docs'])
export const allowedSurfaces = new Set(['visual-ui', 'swagger', 'terminal', 'storybook', 'none'])
export const allowedDispositions = new Set(['integrate', 'discard', 'visualization-only', 'undecided'])
export const allowedStatuses = new Set(['pending', 'in-progress', 'migrated', 'discarded'])
...
function assertEnum(value, allowed, fieldName) {
  assertString(value, fieldName)
  assert(allowed.has(value), `${fieldName} must be one of ${[...allowed].join(', ')} (received "${value}").`)
}
...
function assertTargetToken(value, fieldName, options = {}) {
  ...
  assert(separatorIndex > 0, `${fieldName} must be "visualization-only" or "{rootKey}:{path}" (received "${value}").`)

  const rootKey = value.slice(0, separatorIndex)
  const rest = value.slice(separatorIndex + 1)
  assert(allowedRootKeys.has(rootKey), `${fieldName} has unknown root key "${rootKey}" (expected one of ${[...allowedRootKeys].join(', ')}).`)
  assert(rest.trim().length > 0, `${fieldName} must include a path after "${rootKey}:".`)
}
...
function assertSandboxPath(value, fieldName, sandbox) {
  ...
  assert(value.startsWith(expectedPrefix), `${fieldName} must stay within "${expectedPrefix}" (received "${value}").`)
  assert(!value.split('/').includes('..'), `${fieldName} must not contain ".." path traversal (received "${value}").`)
}
...
function validateCompartment(raw, options) {
  ...
  assertTargetToken(raw.integrationTarget, `${prefix}integrationTarget`, { allowNull: false })
  assertString(raw.addedInConstitutionVersion, `${prefix}addedInConstitutionVersion`)
  assert(/^\d+\.\d+\.\d+$/.test(raw.addedInConstitutionVersion), `${prefix}addedInConstitutionVersion must be a semver "X.Y.Z" string.`)
...
function validateElement(raw, options) {
  ...
  assert(compartmentIds.has(raw.compartment), `${prefix}compartment "${raw.compartment}" does not reference a declared compartment.`)
  ...
  assert(typeof raw.notes === 'string', `${prefix}notes must be a string (may be empty).`)
}
...
export function validateManifest(raw, options = {}) {
  ...
  assert(Array.isArray(raw.compartments), `${prefix}compartments must be an array.`)
  ...
  for (const [index, compartment] of raw.compartments.entries()) {
    validateCompartment(compartment, { prefix: `${prefix}compartments[${index}].`, compartmentIds })
  }
