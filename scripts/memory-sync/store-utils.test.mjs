...
describe("getManifestPath", () => {
  it("returns the manifest path for a workspace and version", () => {
    const path = getManifestPath(
      "/root/.specify/memory/versions",
      "my-project",
      "v1.0.0",
    );
    assert.strictEqual(
      path,
      "/root/.specify/memory/versions/manifests/my-project/v1.0.0.json",
    );
  });
});
...
describe("getTemplatePath", () => {
  it("returns the template path", () => {
    ...
    assert.strictEqual(
      path,
      "/root/.specify/memory/versions/templates/memory-bundle.template.json",
    );
  });
});
...
describe("resolveVersionStorePath", () => {
  it("strips .specify/memory/versions/ prefix", () => {
    const path = resolveVersionStorePath(
      "/root/.specify/memory/versions",
      ".specify/memory/versions/manifests/x/v1.json",
    );
    assert.strictEqual(
      path,
      "/root/.specify/memory/versions/manifests/x/v1.json",
    );
  });
...
describe("resolveExportArtifactPath", () => {
  ...
  it("throws for empty path", () => {
    assert.throws(() => resolveExportArtifactPath(exportsRoot, ""), /required/);
  });
});
