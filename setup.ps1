...
# Orden canónico de los harness — espejo de HARNESS_CANONICAL_ORDER en setup.sh.
# El aviso de reinstalación compara la cadena guardada contra la nueva, así que
# sin un orden fijo `claude,copilot` y `copilot,claude` —la misma elección— se
# reportaban como un cambio de harness.
$script:HarnessCanonicalOrder = @("copilot", "claude", "cursor", "antigravity", "cline")

# Normaliza una selección a una lista canónica separada por comas.
# Acepta comas o espacios, cualquier caja, cualquier orden. `all` gana sobre lo
# demás y elegir los cinco colapsa a `all`. Un nombre desconocido LANZA en vez
# de ignorarse: antes un typo caía al pruning y borraba los cinco en silencio.
function Normalize-HarnessSelection {
    param([string]$Raw)

    $items = @(($Raw -replace ',', ' ') -split '\s+' | Where-Object { $_ } | ForEach-Object { $_.ToLowerInvariant() })
    if ($items.Count -eq 0) { return "all" }

    foreach ($item in $items) {
        if ($item -eq "all") { return "all" }
        if ($script:HarnessCanonicalOrder -notcontains $item) {
            throw "unknown harness: $item"
        }
    }

    $ordered = @($script:HarnessCanonicalOrder | Where-Object { $items -contains $_ })
    if ($ordered.Count -eq $script:HarnessCanonicalOrder.Count) { return "all" }
    return ($ordered -join ',')
}

# ¿La selección incluye este harness? Reemplaza a `-ne` sobre la cadena entera,
# que con `copilot,claude` daba verdadero para AMBOS y borraba justo los dos que
# el operador había pedido.
function Test-HarnessSelected {
    param([string]$Selection, [string]$Candidate)

    if ($Selection -eq "all") { return $true }
    return (@($Selection -split ',') -contains $Candidate)
}
...
function Prune-UnselectedHarnessFiles {
    ...
    foreach ($h in $harnessMap.Keys) {
        if (-not (Test-HarnessSelected -Selection $SelectedHarness -Candidate $h)) {
...
# Valida y normaliza lo que llegó por parámetro ANTES de que nadie lo use.
try {
    $Harness = Normalize-HarnessSelection -Raw $Harness
} catch {
    Write-Err $_.Exception.Message
    Write-Err ("Valid harness names: {0}, all (comma-separated)" -f ($script:HarnessCanonicalOrder -join ', '))
    exit 1
}

# Selección interactiva — espejo del menú de setup.sh, que Windows no tenía en
# absoluto: hasta acá el único camino era pasar -Harness, así que quien instalara
# desde PowerShell nunca pudo elegir. Se pregunta sólo si el operador puede ver
# la pregunta Y contestarla: con la salida redirigida a un log el prompt no se ve
# y esperar la respuesta es un cuelgue.
if (-not [Console]::IsInputRedirected -and -not [Console]::IsOutputRedirected -and -not $PSBoundParameters.ContainsKey('Harness')) {
    Write-Host ""
    Write-Host "🤖 Select the AI assistants to compile rules for:"
    Write-Host ""
    ...
    foreach ($entry in @(
        ...
        $answer = Read-Host ("  {0,-24} [Y/n]" -f ($entry.Label + "?"))
        if ($answer -notmatch '^\s*n(o)?\s*$') { $picked += $entry.Key }
    }

    # Decir que no a los cinco deja un workspace sin ninguna superficie
    # compilada, que no es AOI. La degradación segura es `all`, dicha en voz alta.
    if ($picked.Count -eq 0) {
        Write-Warn "No assistant selected — keeping Universal / All."
        $Harness = "all"
    } else {
        $Harness = Normalize-HarnessSelection -Raw ($picked -join ',')
    }
    Write-Ok "Selected harness: $Harness"
}
