...
import { GENERATED_BANNER, renderManifestMarkdown } from './generate-manifest-md.mjs'
...
test('output always opens with the do-not-edit banner', () => {
  assert.ok(renderManifestMarkdown(skeletonManifest()).startsWith(`${GENERATED_BANNER}\n`))
  assert.ok(renderManifestMarkdown(populatedManifest()).startsWith(`${GENERATED_BANNER}\n`))
})

test('rendering is deterministic across repeated calls', () => {
  assert.equal(renderManifestMarkdown(populatedManifest()), renderManifestMarkdown(populatedManifest()))
  assert.equal(renderManifestMarkdown(skeletonManifest()), renderManifestMarkdown(skeletonManifest()))
})

test('null element targets render as an em dash placeholder', () => {
  const markdown = renderManifestMarkdown(populatedManifest())
  assert.match(markdown, /\| debug-probe \| endpoint \| discard \| discarded \| — \|/)
})
