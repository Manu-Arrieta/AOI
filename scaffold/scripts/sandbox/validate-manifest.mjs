...
async function main() {
  ...
  if (!filePath) {
    process.stderr.write(
      "usage: node validate-manifest.mjs <path-to-integration-manifest.json>\n",
    );
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    process.stdout.write(
      `OK ${filePath}: ${manifest.compartments.length} compartments, ${manifest.elements.length} elements\n`,
    );
  } catch (error) {
