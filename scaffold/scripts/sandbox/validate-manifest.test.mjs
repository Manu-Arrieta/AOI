...
test('enum sets match the locked schema', () => {
  assert.deepEqual([...allowedKinds], ['frontend', 'backend', 'api-contract', 'data', 'infra', 'docs'])
  assert.deepEqual([...allowedSurfaces], ['visual-ui', 'swagger', 'terminal', 'storybook', 'none'])
  assert.deepEqual([...allowedDispositions], ['integrate', 'discard', 'visualization-only', 'undecided'])
  assert.deepEqual([...allowedStatuses], ['pending', 'in-progress', 'migrated', 'discarded'])
})

test('accepts a fully populated manifest', () => {
  assert.doesNotThrow(() => validateManifest(baseManifest(), { filePath: 'm.json' }))
})

test('accepts a skeleton manifest with empty compartments and elements', () => {
  assert.doesNotThrow(() => validateManifest(skeletonManifest(), { filePath: 'm.json' }))
})
...
test('rejects a duplicate element id', () => {
  ...
  assert.throws(() => validateManifest(manifest), /id "auth-form" is duplicated/)
})
...
test('rejects a dangling compartment reference', () => {
  ...
  assert.throws(() => validateManifest(manifest), /does not reference a declared compartment/)
})
